 ...
 */
export function captureStateSnapshot(config: SatoriConfig): StateSnapshotResult | undefined {
  ...
  for (let index = 0; index < config.stateSelectors.length; index++) {
    const selectorConfig: SelectorInput = config.stateSelectors[index] as SelectorInput;
    
    // Handle both old function format and new StateSelector format
    const selector = typeof selectorConfig === 'function' 
      ? selectorConfig 
      : selectorConfig.selector;
    const name = typeof selectorConfig === 'function'
      ? `selector_${index}`
    ...
    try {
      ...
      snapshot[`${name}_error`] = err instanceof Error ? err.message : String(err);
    }
  }
 ...
 */
export function mergeSnapshots(...snapshots: (StateSnapshotResult | undefined)[]): StateSnapshotResult {
