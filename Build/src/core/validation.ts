...
export function validateConfig(config: Partial<SatoriConfig>): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];

  // Validate enableCallsite
  if (config.enableCallsite !== undefined && typeof config.enableCallsite !== 'boolean') {
    errors.push('enableCallsite must be a boolean');
  }

  // Validate enableEnvInfo
  if (config.enableEnvInfo !== undefined && typeof config.enableEnvInfo !== 'boolean') {
    errors.push('enableEnvInfo must be a boolean');
  }

  // Validate enableStateSnapshot
  if (config.enableStateSnapshot !== undefined && typeof config.enableStateSnapshot !== 'boolean') {
    errors.push('enableStateSnapshot must be a boolean');
  }

  // Validate enableCausalLinks
  if (config.enableCausalLinks !== undefined && typeof config.enableCausalLinks !== 'boolean') {
  ...
  if (config.maxBufferSize !== undefined) {
    if (typeof config.maxBufferSize !== 'number') {
      ...
      warnings.push('maxBufferSize is very large (>100000), this may cause memory issues');
    }
  }
  ...
  // Validate appVersion
  if (config.appVersion !== undefined && typeof config.appVersion !== 'string') {
  ...
  if (config.pollingInterval !== undefined) {
    if (typeof config.pollingInterval !== 'number') {
      ...
      warnings.push('pollingInterval is very low (<50ms), this may impact performance');
    }
  }

  // Validate rateLimiting
  if (config.rateLimiting !== undefined) {
    if (typeof config.rateLimiting !== 'object' || config.rateLimiting === null) {
        ...
        if (!Array.isArray(dd.fields)) {
          ...
          dd.fields.forEach((field, index) => {
            if (typeof field !== 'string') {
              ...
              errors.push(`deduplication.fields[${index}] "${field}" is not a valid field. Valid fields: ${validFields.join(', ')}`);
            }
          });
        }
      }
      ...
      config.customLevels.forEach((level, index) => {
        if (typeof level.name !== 'string' || level.name.trim() === '') {
          ...
          if (seenNames.has(level.name)) {
            errors.push(`customLevels[${index}].name "${level.name}" is a duplicate`);
          }
          seenNames.add(level.name);
          
          // Check for completely reserved names (method names)
          if (reservedNames.includes(level.name.toLowerCase())) {
            errors.push(`customLevels[${index}].name "${level.name}" is a reserved method name`);
          }
          
          // Warn if shadowing a built-in level
          if (VALID_LOG_LEVELS.includes(level.name as LogLevel)) {
            warnings.push(`customLevels[${index}].name "${level.name}" shadows a built-in level`);
          }
        }
