...
export function filterByScopePattern(events: LogEntry[], pattern: string | RegExp): LogEntry[] {
...
export function filterByAllTags(events: LogEntry[], tags: string[]): LogEntry[] {
  if (tags.length === 0) return events;
...
export function filterByRegex(events: LogEntry[], pattern: string | RegExp): LogEntry[] {
...
export function filterByRelativeTime(events: LogEntry[], msAgo: number): LogEntry[] {
  const cutoff = Date.now() - msAgo;
  return events.filter(event => event.timestamp >= cutoff);
}

export function filterByCause(events: LogEntry[], causeEventId: string): LogEntry[] {
...
export function filterByStateValue(
  ...
  return events.filter(event => 
    event.state && event.state[key] === value
  );
}
...
export function applyAllFilters(
  ...
  if ('level' in filters && filters.level) {
    filtered = filterByLevel(filtered, filters.level, (filters as FilterOptions).customLevels);
  }
  ...
  if ('scopePattern' in filters && (filters as FilterOptions).scopePattern) {
    filtered = filterByScopePattern(filtered, (filters as FilterOptions).scopePattern!);
  }
  ...
  if ('allTags' in filters && (filters as FilterOptions).allTags && (filters as FilterOptions).allTags!.length > 0) {
